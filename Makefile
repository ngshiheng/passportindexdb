...
docker-build:	## build datasette docker image.
	...
	@if [ -z $(DATASETTE) ]; then echo "Datasette could not be found. See https://docs.datasette.io/en/stable/installation.html"; exit 2; fi
	datasette package $(SQLITE_FILE) --extra-options '--setting allow_download off --setting allow_csv_stream off --setting max_csv_mb 1 --setting default_cache_ttl 86400 --setting sql_time_limit_ms 2000' --metadata data/metadata.json --install=datasette-hashed-urls --install=datasette-block-robots --install=datasette-vega --tag $(IMAGE_NAME):$(TAG_DATE)
